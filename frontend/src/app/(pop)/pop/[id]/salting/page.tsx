...
import SaltingRecordForm from '@/components/pop/process/SaltingRecordForm'
import type { WorkOrder, ResultInput } from '@/types/production'

type Tab = 'result' | 'salting'

export default function SaltingPopPage() {
  ...
  const [ccpViolation, setCcpViolation] = useState(false)
  ...
  return (
    <div className={`space-y-6 ${ccpViolation ? 'outline outline-8 outline-red-500' : ''}`}>
      {/* CCP 이탈 경고 배너 */}
      {ccpViolation && (
        <div className="rounded-2xl border-4 border-red-500 bg-red-600 p-4 text-center">
          <p className="text-xl font-black text-white">CCP 기준값 이탈 감지 — 즉시 조치하십시오</p>
        </div>
      )}
      ...
      <div className="rounded-2xl bg-white p-5 shadow-sm">
        <div className="mb-1 flex items-center gap-2">
          ...
          <span className="rounded-full bg-orange-100 px-3 py-0.5 text-xs font-bold text-orange-700">절임 공정</span>
        </div>
      ...
      <div className="rounded-2xl bg-white shadow-sm">
        <div className="flex border-b border-gray-200">
          ...
          <button
            ...
            className={`flex-1 py-4 text-base font-bold transition-colors ${
              activeTab === 'salting' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'
            }`}
          >
            절임기록
          </button>
