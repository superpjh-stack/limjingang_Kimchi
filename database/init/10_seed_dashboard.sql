-- ============================================================
-- 임진강김치 MES - 대시보드 시드 데이터 (누락 보완 + 오늘 데이터)
-- 오늘 기준: 2026-05-13
-- ============================================================
...
-- ============================================================
-- 1. 공정 기준 (TB_PROCESS) - 누락
-- ============================================================
INSERT IGNORE INTO TB_PROCESS (process_code, process_name, process_type, sequence, description, created_by) VALUES
...
-- ============================================================
-- 2. 창고 (TB_WAREHOUSE) - 누락
-- ============================================================
INSERT IGNORE INTO TB_WAREHOUSE (warehouse_code, warehouse_name, warehouse_type, location, capacity, temp_control, min_temp, max_temp, created_by) VALUES
...
FROM (
    SELECT 'EQP-001' AS equipment_code, '절임탱크 1호' AS equipment_name, 'TANK' AS equipment_type, 'PRC-004' AS proc_code, 2000.00 AS capacity, '절임실 A구역' AS location
    UNION ALL SELECT 'EQP-002','절임탱크 2호','TANK','PRC-004',2000.00,'절임실 A구역'
    UNION ALL SELECT 'EQP-003','절임탱크 3호','TANK','PRC-004',1000.00,'절임실 B구역'
    UNION ALL SELECT 'EQP-004','세척라인 1호','LINE','PRC-003',500.00,'세척실'
    UNION ALL SELECT 'EQP-005','양념버무림기 1호','MACHINE','PRC-006',300.00,'양념실 A구역'
    UNION ALL SELECT 'EQP-006','양념버무림기 2호','MACHINE','PRC-006',300.00,'양념실 A구역'
    UNION ALL SELECT 'EQP-007','자동포장기 1호','MACHINE','PRC-007',200.00,'포장실'
    UNION ALL SELECT 'EQP-008','자동포장기 2호','MACHINE','PRC-007',200.00,'포장실'
    UNION ALL SELECT 'EQP-009','절임탱크 4호','TANK','PRC-004',1500.00,'절임실 B구역'
    UNION ALL SELECT 'EQP-010','세척라인 2호','LINE','PRC-003',500.00,'세척실'
    UNION ALL SELECT 'EQP-011','세척라인 3호','LINE','PRC-003',300.00,'세척실'
    UNION ALL SELECT 'EQP-012','양념버무림기 3호','MACHINE','PRC-006',300.00,'양념실 B구역'
    UNION ALL SELECT 'EQP-013','양념버무림기 4호','MACHINE','PRC-006',200.00,'양념실 B구역'
    UNION ALL SELECT 'EQP-014','자동포장기 3호','MACHINE','PRC-007',200.00,'포장실'
    UNION ALL SELECT 'EQP-015','자동포장기 4호','MACHINE','PRC-007',150.00,'포장실'
) AS eq
...
-- 4. BOM (TB_BOM) - 누락
-- ============================================================
INSERT IGNORE INTO TB_BOM (bom_code, product_id, bom_name, total_qty, version, effective_date, description, created_by)
SELECT 'BOM-001', id, '율무 포기김치 100kg 레시피', 100.00, '1.0', CURDATE(), '배추김치 기준 레시피', 'SYSTEM' FROM TB_PRODUCT WHERE product_code = 'PRD-001';

INSERT IGNORE INTO TB_BOM (bom_code, product_id, bom_name, total_qty, version, effective_date, description, created_by)
SELECT 'BOM-002', id, '율무 총각김치 100kg 레시피', 100.00, '1.0', CURDATE(), '총각김치 기준 레시피', 'SYSTEM' FROM TB_PRODUCT WHERE product_code = 'PRD-004';

-- ============================================================
-- 5. 추가 거래처 (CUS-006 ~ CUS-020) - 누락
-- ============================================================
...
FROM (
    ...
    UNION ALL SELECT 'ORD-20260510-001','CUS-001','2026-05-10','2026-05-17',4900000,'CONFIRMED','HOMESHOPPING','CJ온스타일 5월 1차 방송'
    UNION ALL SELECT 'ORD-20260512-001','CUS-019','2026-05-12','2026-05-19',1400000,'CONFIRMED','GENERAL','GS25 편의점 발주'
    UNION ALL SELECT 'ORD-20260513-001','CUS-002','2026-05-13','2026-05-20',3600000,'DRAFT','HOMESHOPPING','GS홈쇼핑 5월 2차'
    UNION ALL SELECT 'ORD-20260513-002','CUS-013','2026-05-13','2026-05-16',1260000,'DRAFT','ONLINE','마켓컬리 5월 2주차'
) AS ord
...
SELECT o.id, p.id,
    CASE o.order_no
        WHEN 'ORD-20260401-001' THEN 100 WHEN 'ORD-20260405-001' THEN 80
        WHEN 'ORD-20260408-001' THEN 30  WHEN 'ORD-20260412-001' THEN 120
        WHEN 'ORD-20260415-001' THEN 90  WHEN 'ORD-20260417-001' THEN 36
        WHEN 'ORD-20260420-001' THEN 48  WHEN 'ORD-20260425-001' THEN 120
        WHEN 'ORD-20260428-001' THEN 80  WHEN 'ORD-20260501-001' THEN 94
        WHEN 'ORD-20260503-001' THEN 56  WHEN 'ORD-20260505-001' THEN 36
        WHEN 'ORD-20260506-001' THEN 108 WHEN 'ORD-20260507-001' THEN 24
        WHEN 'ORD-20260508-001' THEN 29  WHEN 'ORD-20260509-001' THEN 60
        WHEN 'ORD-20260510-001' THEN 140 WHEN 'ORD-20260512-001' THEN 40
        WHEN 'ORD-20260513-001' THEN 103 WHEN 'ORD-20260513-002' THEN 36
        ELSE 40
    END AS order_qty,
    p.unit_price,
    CASE o.order_no
...
WHERE o.order_no IN (
    'ORD-20260401-001','ORD-20260405-001','ORD-20260408-001','ORD-20260412-001',
    'ORD-20260415-001','ORD-20260417-001','ORD-20260420-001','ORD-20260425-001',
    'ORD-20260428-001','ORD-20260501-001','ORD-20260503-001','ORD-20260505-001',
    'ORD-20260506-001','ORD-20260507-001','ORD-20260508-001','ORD-20260509-001',
    'ORD-20260510-001','ORD-20260512-001','ORD-20260513-001','ORD-20260513-002'
);
...
FROM (
    ...
    UNION ALL SELECT 'PLAN-20260407-001','2026-04-07','PRD-004',300,297,'COMPLETED','DAILY','2026-04-07 07:00:00','2026-04-07 16:00:00','총각김치 납품분'
    ...
    UNION ALL SELECT 'PLAN-20260410-001','2026-04-10','PRD-006',200,195,'COMPLETED','DAILY','2026-04-10 06:00:00','2026-04-10 18:00:00','열무김치 소량'
    ...
    UNION ALL SELECT 'PLAN-20260421-001','2026-04-21','PRD-004',250,252,'COMPLETED','DAILY','2026-04-21 07:00:00','2026-04-21 15:00:00','총각김치'
    UNION ALL SELECT 'PLAN-20260425-001','2026-04-25','PRD-006',180,179,'COMPLETED','DAILY','2026-04-25 07:00:00','2026-04-25 15:00:00','열무김치 납품'
    ...
    UNION ALL SELECT 'PLAN-20260501-001','2026-05-01','PRD-001',500,498,'COMPLETED','DAILY','2026-05-01 06:00:00','2026-05-01 19:00:00','GS홈쇼핑 5월'
    UNION ALL SELECT 'PLAN-20260505-001','2026-05-05','PRD-004',300,296,'COMPLETED','DAILY','2026-05-05 06:00:00','2026-05-05 17:00:00','총각김치 5월'
    UNION ALL SELECT 'PLAN-20260507-001','2026-05-07','PRD-002',400,402,'COMPLETED','DAILY','2026-05-07 06:00:00','2026-05-07 18:00:00','홈플러스 10kg'
    UNION ALL SELECT 'PLAN-20260508-001','2026-05-08','PRD-001',600,594,'COMPLETED','DAILY','2026-05-08 06:00:00','2026-05-08 20:00:00','현대홈쇼핑 5월'
    UNION ALL SELECT 'PLAN-20260509-001','2026-05-09','PRD-006',200,198,'COMPLETED','DAILY','2026-05-09 07:00:00','2026-05-09 16:00:00','열무김치 5월'
    UNION ALL SELECT 'PLAN-20260510-001','2026-05-10','PRD-001',700,703,'COMPLETED','DAILY','2026-05-10 06:00:00','2026-05-10 21:00:00','CJ온스타일 5월'
    UNION ALL SELECT 'PLAN-20260512-001','2026-05-12','PRD-004',350,345,'COMPLETED','DAILY','2026-05-12 06:00:00','2026-05-12 17:00:00','총각김치 5월 2차'
    UNION ALL SELECT 'PLAN-20260513-001','2026-05-13','PRD-001',800,320,'IN_PROGRESS','DAILY','2026-05-13 06:00:00','2026-05-13 21:00:00','GS홈쇼핑 5월 2차'
    UNION ALL SELECT 'PLAN-20260513-002','2026-05-13','PRD-004',300,0,'IN_PROGRESS','DAILY','2026-05-13 08:00:00','2026-05-13 18:00:00','총각김치 오늘 생산'
...
FROM (
    ...
    UNION ALL SELECT 'WO-20260403-001','PLAN-20260403-001','PRD-001','PRC-007','EQP-007','worker2','2026-04-03',600,601,0,'COMPLETED','2026-04-03 09:00:00','2026-04-03 19:30:00','LOT-20260403-001','manager','worker2'
    UNION ALL SELECT 'WO-20260407-001','PLAN-20260407-001','PRD-004','PRC-006','EQP-006','worker3','2026-04-07',300,297,3,'COMPLETED','2026-04-07 08:00:00','2026-04-07 15:00:00','LOT-20260407-001','manager','worker3'
    UNION ALL SELECT 'WO-20260408-001','PLAN-20260408-001','PRD-001','PRC-007','EQP-008','worker4','2026-04-08',400,403,0,'COMPLETED','2026-04-08 08:00:00','2026-04-08 17:00:00','LOT-20260408-001','manager','worker4'
    UNION ALL SELECT 'WO-20260410-001','PLAN-20260410-001','PRD-006','PRC-006','EQP-005','worker1','2026-04-10',200,195,5,'COMPLETED','2026-04-10 08:00:00','2026-04-10 17:30:00','LOT-20260410-001','manager','worker1'
    UNION ALL SELECT 'WO-20260414-001','PLAN-20260414-001','PRD-001','PRC-006','EQP-005','worker2','2026-04-14',700,695,5,'COMPLETED','2026-04-14 07:00:00','2026-04-14 20:00:00','LOT-20260414-001','manager','worker2'
    ...
    UNION ALL SELECT 'WO-20260421-001','PLAN-20260421-001','PRD-004','PRC-007','EQP-007','worker4','2026-04-21',250,252,0,'COMPLETED','2026-04-21 08:00:00','2026-04-21 15:00:00','LOT-20260421-001','manager','worker4'
    UNION ALL SELECT 'WO-20260425-001','PLAN-20260425-001','PRD-006','PRC-007','EQP-008','worker1','2026-04-25',180,179,1,'COMPLETED','2026-04-25 08:00:00','2026-04-25 15:00:00','LOT-20260425-001','manager','worker1'
    UNION ALL SELECT 'WO-20260428-001','PLAN-20260428-001','PRD-001','PRC-007','EQP-007','worker2','2026-04-28',600,600,0,'COMPLETED','2026-04-28 07:00:00','2026-04-28 19:30:00','LOT-20260428-001','manager','worker2'
    UNION ALL SELECT 'WO-20260501-001','PLAN-20260501-001','PRD-001','PRC-006','EQP-005','worker3','2026-05-01',500,498,2,'COMPLETED','2026-05-01 08:00:00','2026-05-01 19:00:00','LOT-20260501-001','manager','worker3'
    UNION ALL SELECT 'WO-20260505-001','PLAN-20260505-001','PRD-004','PRC-006','EQP-006','worker4','2026-05-05',300,296,4,'COMPLETED','2026-05-05 08:00:00','2026-05-05 17:00:00','LOT-20260505-001','manager','worker4'
    UNION ALL SELECT 'WO-20260507-001','PLAN-20260507-001','PRD-002','PRC-006','EQP-012','worker1','2026-05-07',400,402,0,'COMPLETED','2026-05-07 08:00:00','2026-05-07 18:00:00','LOT-20260507-001','manager','worker1'
    UNION ALL SELECT 'WO-20260508-001','PLAN-20260508-001','PRD-001','PRC-006','EQP-005','worker2','2026-05-08',600,594,6,'COMPLETED','2026-05-08 07:00:00','2026-05-08 20:00:00','LOT-20260508-001','manager','worker2'
    UNION ALL SELECT 'WO-20260509-001','PLAN-20260509-001','PRD-006','PRC-007','EQP-008','worker3','2026-05-09',200,198,2,'COMPLETED','2026-05-09 08:00:00','2026-05-09 16:00:00','LOT-20260509-001','manager','worker3'
    UNION ALL SELECT 'WO-20260510-001','PLAN-20260510-001','PRD-001','PRC-006','EQP-005','worker4','2026-05-10',700,703,0,'COMPLETED','2026-05-10 07:00:00','2026-05-10 21:00:00','LOT-20260510-001','manager','worker4'
    UNION ALL SELECT 'WO-20260512-001','PLAN-20260512-001','PRD-004','PRC-007','EQP-007','worker1','2026-05-12',350,345,5,'COMPLETED','2026-05-12 08:00:00','2026-05-12 17:00:00','LOT-20260512-001','manager','worker1'
    -- 오늘 (2026-05-13) 진행 중인 작업지시
    UNION ALL SELECT 'WO-20260513-001','PLAN-20260513-001','PRD-001','PRC-006','EQP-005','worker2','2026-05-13',800,320,4,'IN_PROGRESS','2026-05-13 06:00:00',NULL,'LOT-20260513-001','manager',NULL
    UNION ALL SELECT 'WO-20260513-002','PLAN-20260513-001','PRD-001','PRC-007','EQP-014','worker3','2026-05-13',800,0,0,'ISSUED',NULL,NULL,'LOT-20260513-001','manager',NULL
    UNION ALL SELECT 'WO-20260513-003','PLAN-20260513-002','PRD-004','PRC-006','EQP-006','worker4','2026-05-13',300,0,0,'ISSUED',NULL,NULL,'LOT-20260513-002','manager',NULL
) AS wo
...
-- ============================================================
-- 10. 자재재고 (TB_MATERIAL_STOCK) - 현재 재고
-- ============================================================
...
-- ============================================================
-- 11. 완제품재고 (TB_PRODUCT_STOCK) - 현재 재고
-- ============================================================
...
FROM (
    ...
    UNION ALL SELECT 'SHP-20260404-001','ORD-20260403-001','CUS-002','2026-04-04','DELIVERED','롯데글로벌로지스','LG2026040400001' -- 이 주문 없어도 IGNORE됨
    ...
    UNION ALL SELECT 'SHP-20260513-001','ORD-20260513-001','CUS-002','2026-05-13','PREPARING','롯데글로벌로지스','LG2026051300001'
    UNION ALL SELECT 'SHP-20260513-002','ORD-20260513-002','CUS-013','2026-05-13','PREPARING','마켓컬리배송','MK2026051300001'
) AS sh
...
WHERE s.shipment_no IN (
    'SHP-20260402-001','SHP-20260409-001','SHP-20260413-001','SHP-20260416-001',
    'SHP-20260418-001','SHP-20260421-001','SHP-20260426-001','SHP-20260429-001',
    'SHP-20260502-001','SHP-20260504-001','SHP-20260506-001','SHP-20260507-001',
    'SHP-20260508-001','SHP-20260509-001','SHP-20260510-001','SHP-20260511-001',
    'SHP-20260512-001','SHP-20260513-001','SHP-20260513-002'
);

-- ============================================================
-- 확인
-- ============================================================
SELECT '=== 시드 완료 ===' AS info;
SELECT 'TB_PROCESS' AS tbl, COUNT(*) AS cnt FROM TB_PROCESS
UNION ALL SELECT 'TB_WAREHOUSE', COUNT(*) FROM TB_WAREHOUSE
UNION ALL SELECT 'TB_EQUIPMENT', COUNT(*) FROM TB_EQUIPMENT
